...
// Post a comment to an individual post
router.post('/post/:id/comment', isAuthenticated, async (req, res) => {
  const postId = req.params.id;
  const { comment } = req.body; // The comment text
  const userId = req.user.id; // The ID of the user posting the comment

  try {
    await prisma.comment.create({
      data: {
        body: comment,
        postId: postId, // Directly link the comment to the post
        userId: userId, // Directly link the comment to the user
      },
    });
...
// DELETE route for comment deletion
router.post('/comment/:commentId/delete', isAuthenticated, async (req, res) => {
  ...
  try {
    // Use Prisma to delete the comment
